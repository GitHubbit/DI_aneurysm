...
**Plot changes in expression in inflammatory genes under different conditions. This can be repeated for any set of genes in a biological pathway of interest (TGF-beta, etc.)**

```{r inflammation}

inflammation <- ggplot(ridge.by.path$inflammation,
                       aes(x = ridge.by.path$inflammation$expression_level,
                           y = ridge.by.path$inflammation$gene,
                           fill = ridge.by.path$inflammation$Group,
                           color = ridge.by.path$inflammation$Group)) + 
  geom_density_ridges(scale = 0.8, size = 0.5, alpha = 0.2, rel_min_height = 0.01) + 
  scale_fill_manual(
    name = "Conditions",
...
inflammation
```
**Plots imply that there are both physiological and molecular responses to of aneurysms to treatment. The tSNE shows that cells Futher investigation on**
